...
import hbuild = require("../../generic/hbuild");
...
export var JQUERY_MENUE_ITEMS_EXPRS = "ul.navbar-nav";

export interface IBootStrapNavBarOpt {
    menueCaption: string;
    menueItems: IMenuItem[];
}

export function genBootStrapNavBar(o: IBootStrapNavBarOpt) {
    ...
    function buildNavBarHeaderToggleButton(sb, cbData) {
        ...
        var arr_spans:hbuild.IfnOnInner[] = [buildSpanSrOnly];
    ...
    function buildMenuItem(sb, cbData) {
        ...
        var mi: IMenuItem = cbData.mi;
        var href = mi.href || '#';
        var arr_str_attr: string[] = ["href", href, "id", mi.id];        
    ...
    function buildMenuItems(sb, cbData) {
        // <ul class="nav navbar-nav" >
        o.menueItems.forEach((mi:IMenuItem)=> {
...
export interface IActionMap {
    // definiert eine Map mit String indexiert
    [id: string]: IMenuAction;
}

export function setupMenu(actionmap: IActionMap) {
    var j_menuItems = $(JQUERY_MENUE_ITEMS_EXPRS);
    if (j_menuItems.length == 0) {
        throw new Error("konnte Menuitems expr nicht finden");
    }
    j_menuItems.bind("click",(e: JQueryEventObject) => {
